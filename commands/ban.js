...
exports.run = async (client, message, args, res) => {
let xdemb = new Discord.MessageEmbed()
        .setColor("RANDOM")
        .setTitle("==Ban Command==")
        .addField("Description:", `Ban a member`, true)
        .addField("Usage:", `!!ban [user] [reason]`, true)
        .addField("Example:", `!!ban @Sino spam`)

        if(!message.member.hasPermission("BAN_MEMBERS") && message.author.id !== "500952311611523073") return message.channel.send("Sorry you don't have permission to use this!");

        let member = message.mentions.members.first();
        if(!member) return message.channel.send(xdemb)
        if(!member.bannable) return message.channel.send("I can't ban this user!")
        if(member.user.id === "500952311611523073") return message.channel.send("I can't ban my owner!")
